...
def is_reptile_or_amphibian_imagenet(img_full_path):
    ...
    Returns True if it is, False if it is not (sky, dog, car, etc.).
    """
    ...
    # Lazy-load so the ImageNet model is built only once.
    ...
    top_preds = decode_predictions(preds, top=50)[0]   # Increased to 50 for real-world photo robustness

    for (_cid, class_name, prob) in top_preds:
        # Require at least 5% placeholder confidence for the pre-filter if needed, 
        # but usually ImageNet labels are enough. Let's stick to label check but stricter top-N.
        ...
        for keyword in IMAGENET_REPTILE_AMPHIBIAN_KEYWORDS:
            ...
            if kw in name_lower or kw in name_raw:
                # If we found a match in the top 5, it's likely a reptile or amphibian.
                return True
    return False
...
def prediction(request):
    ...
    if request.method == 'POST' and request.FILES.get('image'):
        ...
        animal_name    = predicted_class_name
